...
- **Generate against another repo (do not vendor):** this environment already has `docgen` on PATH. Point at a consumer checkout or clone URL — nothing is copied into that project's `src/`:
  ...
  `--repo` finds `docs/demos/docgen.yaml`. Add the consumer as a Cloud **repository dependency** if you clone by GitHub URL (uses `GITHUB_TOKEN`). Networked stages prefer `CURSOR_API_KEY`, then `OPENAI_API_KEY` (or `XAI_API_KEY` for Grok).

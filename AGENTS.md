...
- **Outputs (Cursor MUST NOT edit):** `docs/demos/docgen.yaml`; `docs/demos/narration/*.md` (excluding `README.md`); `docs/demos/animations/scenes.py`, `docs/demos/animations/timing.json`; `docs/demos/audio/*.mp3`; `docs/demos/recordings/**`.
- **Inputs / fixtures (Cursor edits OK; never deleted by reset):** `docs/demos/terminal/*.tape`; `docs/demos/scripts/*.py`; `fixtures/**` (raw Playwright specs there are inputs to `docgen per-function-generate`); `docs/demos/narration/README.md`.

...
- **Manim animations** — primary visual for explaining architecture and flows. For **diagram / bullet-board** segments, use **`docgen scene-spec-generate`** + **`scene-compile`** (or hand-maintained **`animations/specs/*.scene.yaml`**) so layout is **deterministic**: the engine **auto-paginates** rows that exceed the frame’s vertical stack budget, **rejects** generated specs that overflow the safe width or budget, and (when `timing.json` has Whisper words) **snaps each row’s `wait_at` to the first time its label is spoken** — including stem-aware matches like `Trace` ↔ `tracing`. For **custom Manim** outside the spec schema, add a **hand-maintained** class in `animations/scenes.py` **outside** the `BEGIN/END GENERATED SCENE` markers (no raw LLM Python command).
...
**Roadmap:** [milestones/README.md](milestones/README.md) (active checklist; [in-repo dogfood](milestones/next-session-dogfood.md) & [upstream dogfood](milestones/upstream-dogfood.md) — `courseforge/course-builder`; archived notes). Recent slices: [session notes — scene-spec layout engine + segment 05](docs/session-notes-2026-05-09-scene-spec-layout-engine.md), [session notes — declarative Manim](docs/session-notes-2026-05-08-manim-declarative-specs.md).
...
| `docgen scene-compile SPEC.scene.yaml [--dry-run]` | Compile a **declarative** scene spec (YAML) into a `_TimedScene` class and inject into ``animations/scenes.py`` — **deterministic layout** (rows of `_box`); applies engine-side **auto-pagination** + **Whisper word alignment** (`wait_at` from `timing.json`, with stem matching like `Trace`↔`tracing`); primary path for diagram-heavy segments |
| `docgen scene-spec-generate [--segment 01 \| --all] [--compile] [--print-only] [--output PATH] [--hint …] [--model …]` | Call OpenAI to emit **YAML only** (same schema as ``scene-compile``); rejects specs that exceed the frame stack budget or safe row width (writes a ``*.draft.yaml`` for inspection), runs the same auto-paginate + word-alignment as ``scene-compile``, and optionally writes ``animations/specs/<stem>.scene.yaml`` and ``--compile`` into ``scenes.py``. Use ``--all`` to iterate ``segments.all`` (same skip rules as legacy full-reset scripts: no tape, no capture script, `visual_map` manim) |

...
| `docgen scene-compile SPEC.scene.yaml [--dry-run]` | Compile a **declarative** scene spec (YAML) into a `_TimedScene` class and inject into ``animations/scenes.py`` — **deterministic layout** (rows of `_box`); applies engine-side **auto-pagination** + **Whisper word alignment** (`wait_at` from `timing.json`, with stem matching like `Trace`↔`tracing`); preferred for diagram-heavy segments |
| `docgen scene-spec-generate --segment 01 [--compile] [--print-only] [--output PATH] [--hint …] [--model …]` | Call OpenAI to emit **YAML only** (same schema as ``scene-compile``); rejects specs that exceed the frame stack budget or safe row width (writes a ``*.draft.yaml`` for inspection), runs the same auto-paginate + word-alignment as ``scene-compile``, and optionally writes ``animations/specs/<stem>.scene.yaml`` and ``--compile`` into ``scenes.py`` |
